...
void CMy150716MapToolmfcView::OnDraw(CDC* /*pDC*/)
{
	...
	for(int i = 0; i<18; ++i)
	{
		for (int j =0; j<6; ++j)
		{
			...
			D3DXMATRIX matWorld;
			D3DXMATRIX matTrans;
			D3DXMatrixTranslation(&matTrans,m_vecTile[nIndex]->vPos.x, m_vecTile[nIndex]->vPos.y, m_vecTile[nIndex]->vPos.z);
...
void CMy150716MapToolmfcView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	D3DXVECTOR2 vMousePoint = D3DXVECTOR2(point.x, point.y);
